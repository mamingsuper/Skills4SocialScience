# Skills for Social Science 🎓🏇

<div align="center">

![GitHub stars](https://img.shields.io/github/stars/mamingsuper/Skills4SocialScience?style=social)
![GitHub forks](https://img.shields.io/github/forks/mamingsuper/Skills4SocialScience?style=social)
![License](https://img.shields.io/badge/license-MIT-blue.svg)

**A Curated Collection of AI Skills & Tools for Social Science Research**

[English](README.md) | [中文](README_CN.md)

</div>

**🌐 [Project Website](https://mamingsuper.github.io/Skills4SocialScience)**

--- 

## 📋 Table of Contents

- [About](#about)
- [Research Workflow](#research-workflow)
- [Skills by Category](#skills-by-category)
- [Getting Started](#getting-started)
- [Contributing](#contributing)
- [External Resources](#external-resources)
- [License](#license)

---

## 🎯 About

This repository contains a comprehensive collection of AI-powered skills and tools specifically designed for social science scholars, researchers, and students. Whether you're conducting literature reviews, analyzing data, or preparing publications, you'll find valuable resources organized by research workflow stages.

**Key Features:**
- ✅ Organized by research workflow stages
- ✅ Tailored for social science methodologies
- ✅ AI-powered tools and automation
- ✅ Bilingual documentation (English/Chinese)
- ✅ Regularly updated with community contributions

---

## 🔬 Research Workflow

Our skills are organized following the typical social science research workflow:

```
🎯 One-Stop Solutions (Integrated multi-stage tools)
        ↓
Literature Review → Research Design → Data Collection → Data Analysis → Writing & Publishing → Presentation → Teaching
```

---

## 📚 Skills by Category

### 🎯 One-Stop Solutions
**Purpose:** Comprehensive integrated tools that cover multiple research stages

<details>
<summary><b>Available Solutions</b></summary>

- **[Claude Scientific Writer](_skills/writing/claude-writer/SKILL.md)** 📝
  - End-to-end academic paper writing solution
  - Integrated workflow from outline to final draft
  - Supports multiple scientific and social science disciplines
  - AI-powered writing assistance with Claude
  - Literature integration and citation management
  - Structured academic paper formats (IMRAD)
  - [View resource guide](_skills/writing/claude-writer/SKILL.md)
  - [Visit GitHub repository](https://github.com/K-Dense-AI/claude-scientific-writer)

</details>

---

### 1️⃣ Literature Review & Pre-Research
**Purpose:** Finding, organizing, and synthesizing existing research

<details>
<summary><b>Available Skills</b></summary>

*Skills coming soon. Contribute your literature review tools to help the community!*

</details>

---

### 2️⃣ Research Design & Planning
**Purpose:** Designing robust research frameworks

<details>
<summary><b>Available Skills & Resources</b></summary>

- **[Open Science Skills](_skills/design/open-science-skills/SKILL.md)** 🔬
  - Comprehensive collection of open science practices
  - Reproducible research workflows
  - Data management and sharing protocols
  - Pre-registration guidelines
  - Version control for research
  - [View resource guide](_skills/design/open-science-skills/SKILL.md)
  - [Visit GitHub repository](https://github.com/scdenney/open-science-skills)

</details>

---

### 3️⃣ Data Collection
**Purpose:** Gathering and managing research data

<details>
<summary><b>Available Skills</b></summary>

*Skills coming soon. Contribute your data collection tools to help the community!*

</details>

---

### 4️⃣ Data Analysis
**Purpose:** Analyzing quantitative and qualitative data

<details>
<summary><b>Available Skills</b></summary>

*Skills coming soon. Contribute your data analysis tools to help the community!*

</details>

---

### 5️⃣ Writing & Publishing
**Purpose:** Crafting and publishing research outputs

<details>
<summary><b>Available Skills</b></summary>

*Skills coming soon. Contribute your writing and publishing tools to help the community!*

</details>

---

### 6️⃣ Presentation & Dissemination
**Purpose:** Communicating research findings effectively

<details>
<summary><b>Available Skills</b></summary>

*Skills coming soon. Contribute your presentation and dissemination tools to help the community!*

</details>

---

### 7️⃣ Teaching & Pedagogy
**Purpose:** Supporting teaching and knowledge transfer

<details>
<summary><b>Available Skills</b></summary>

*Skills coming soon. Contribute your teaching and pedagogy tools to help the community!*

</details>

---

## 🚀 Getting Started

### Prerequisites

To use these skills, you'll need:
- Claude Code CLI installed ([Installation Guide](https://docs.anthropic.com/claude-code))
- Basic familiarity with command-line tools
- Access to Claude AI API (for some skills)

### Installation

1. **Clone this repository:**
   ```bash
   git clone https://github.com/mamingsuper/Skills4SocialScience.git
   cd Skills4SocialScience
   ```

2. **Browse available skills:**
   ```bash
   find _skills -name "SKILL.md"
   ```

3. **Install a specific skill:**
   - Follow the installation instructions in each skill's directory
   - Most skills can be installed via Claude Code CLI

4. **Use the skill:**
   ```bash
   # Example: Using the humanizer skill
   /humanizer "Your text here"
   ```

### Quick Start Guide

New to AI-powered research tools? Start here:

1. **For Complete Paper Writing:** Try [Claude Scientific Writer](_skills/writing/claude-writer/SKILL.md) for an integrated academic writing solution
2. **For Research Design:** Explore [Open Science Skills](_skills/design/open-science-skills/SKILL.md) to learn reproducible research practices
3. **Contribute Skills:** Help build this repository by adding your own research tools and workflows
4. **Join the Community:** Share your experiences and learn from other social science researchers

---

## 🤝 Contributing

We welcome contributions from the social science community! Here's how you can help:

### Adding New Skills

1. Fork this repository
2. Create a new branch (`git checkout -b feature/new-skill`)
3. Add your skill to the appropriate category folder
4. Include detailed documentation (see [template](legacy_docs/guides/skill-template.md))
5. Submit a pull request

### Reporting Issues

Found a bug or have a suggestion? Please [open an issue](https://github.com/mamingsuper/Skills4SocialScience/issues).

### Skill Submission Guidelines

- **Documentation:** Include clear usage examples and prerequisites
- **Testing:** Ensure your skill works across different environments
- **Licensing:** All contributions must be compatible with the MIT license
- **Quality:** Follow academic integrity and research ethics standards

---

## 🌐 External Resources

### Recommended Tools & Integrations

- **[Claude Scientific Writer](https://github.com/K-Dense-AI/claude-scientific-writer):** One-stop solution for academic paper writing
- **[Open Science Skills](https://github.com/scdenney/open-science-skills):** Comprehensive open science practices and workflows
- **NotebookLM:** AI-powered research notebook
- **Claude Code:** AI coding assistant for research automation
- **Zotero/Mendeley:** Reference management tools
- **OSF (Open Science Framework):** Research project management and sharing

---

## 📖 Documentation

Detailed reference docs:

- [Skill Development Guide](legacy_docs/guides/skill-development.md)
- [Skill Template Guide](legacy_docs/guides/skill-template.md)
- [Online Learn Page](https://mamingsuper.github.io/Skills4SocialScience/pages/learn.html)

---

## 🌐 Featured Resource

- **[OpenResearcher](https://github.com/TIGER-AI-Lab/OpenResearcher):** TIGER AI Lab's curated set of reproducible research workflows, knowledge graphs, and LLM-friendly experiments tailored for social scientists.

---

## 📜 License

This project is licensed under the MIT License - see the [LICENSE](LICENSE) file for details.

---

## 🙏 Acknowledgments

- Thanks to all contributors who have helped build this collection
- Inspired by the open science movement and reproducible research practices
- Special thanks to the Claude AI and Anthropic teams

---

## 📬 Contact & Support

- **Issues:** [GitHub Issues](https://github.com/mamingsuper/Skills4SocialScience/issues)
- **Repository:** [GitHub Repo](https://github.com/mamingsuper/Skills4SocialScience)
- **Website:** [Skills4SocialScience](https://mamingsuper.github.io/Skills4SocialScience)

## 👤 About the Maintainer

- Website: [maming.digital](https://www.maming.digital/)
- X/Twitter: [Winter62675779](https://x.com/Winter62675779)
- WeChat: 扫码关注（二维码下方）  

![WeChat QR](assets/images/wechat-qrcode.jpg)

---

<div align="center">

**⭐ If you find this repository useful, please consider giving it a star! ⭐**

Made with ❤️ by the social science research community

</div>
